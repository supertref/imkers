...
Blockchain::Blockchain(const Currency& currency, tx_memory_pool& tx_pool, ILogger& logger, bool blockchainIndexesEnabled, int poolSize) :
...
bool Blockchain::init(const std::string& config_folder, bool load_existing) {
  ...
  if (!m_blocks.open(appendPath(config_folder, m_currency.blocksFileName()), appendPath(config_folder, m_currency.blockIndexesFileName()), m_poolSize)) {
    return false;
  }
...
bool Blockchain::storeCache() {
  ...
  logger(INFO, BRIGHT_WHITE) << "Saving blockchain at height " << m_blocks.size() << ".";
...
difficulty_type Blockchain::getDifficultyForNextBlock() {
  ...
  return m_currency.nextDifficulty(BlockMajorVersion, timestamps, commulative_difficulties, m_blocks.size() - 1);
}
...
difficulty_type Blockchain::get_next_difficulty_for_alternative_chain(const std::list<blocks_ext_by_hash::iterator>& alt_chain, BlockEntry& bei) {
  ...
  return m_currency.nextDifficulty(BlockMajorVersion, timestamps, commulative_difficulties, currentHeight);
}
